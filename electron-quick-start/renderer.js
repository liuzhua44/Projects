...
var printJson = new Array();
createPrintAccount = function(){
    for(var i=0;i<importJson.length;i++){
        var info = importJson[i];
        var row = new Array();
        row.push(info[5]);
        row.push(info[2]);
        row.push(info[4]);
    ...
    for(var i=0;i<printJson.length;i++){
        var html = '<tr>';
        for(var j=0;j<printJson[i].length;j++){
            html += '<td>' + printJson[i][j] + '</td>';
        }
        html += '</tr>';
        $("#gaPrintTable").append(html);
    }
}
...
saveImportExcle = function(){
    console.log("saveImportExcle")
    var sheet = XLSX.utils.aoa_to_sheet(importJson);
    openDownloadDialog(sheet2blob(sheet),"导入账号.xlsx")
}
